...
impl NetworkDefinition {
    ...
    pub fn add_convolution(
        ...
        #[cfg(not(feature = "mock"))]
        {
            ...
            if tensor_ptr.is_null() {
                return Err(Error::Runtime("Failed to add convolution layer".to_string()));
            }
            Ok(Tensor { inner: tensor_ptr })
        }
        #[cfg(feature = "mock")]
    ...
    pub fn add_concatenation(&mut self, inputs: &[&Tensor]) -> Result<Tensor> {
        #[cfg(not(feature = "mock"))]
        {
            ...
            if tensor_ptr.is_null() {
                return Err(Error::Runtime("Failed to add concatenation layer".to_string()));
            }
            Ok(Tensor { inner: tensor_ptr })
        }
        #[cfg(feature = "mock")]
    ...
    pub fn add_softmax(&mut self, input: &Tensor, axes: u32) -> Result<Tensor> {
        ...
        {
            let tensor_ptr = unsafe {
                trtx_sys::network_add_softmax(self.inner, input.inner, axes)
            };
            if tensor_ptr.is_null() {
    ...
    pub fn add_resize(&mut self, input: &Tensor) -> Result<Tensor> {
        ...
        {
            let tensor_ptr = unsafe {
                trtx_sys::network_add_resize(self.inner, input.inner)
            };
            if tensor_ptr.is_null() {
    ...
    pub fn add_topk(&mut self, input: &Tensor, op: i32, k: i32, axes: u32) -> Result<Tensor> {
        ...
        {
            let tensor_ptr = unsafe {
                trtx_sys::network_add_topk(self.inner, input.inner, op, k, axes)
            };
            if tensor_ptr.is_null() {
